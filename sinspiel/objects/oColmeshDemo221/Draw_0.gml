...
var ray = cm_cast_ray(LEVEL_COLMESH, cm_ray(global.camX, global.camY, global.camZ, global.camX + v[0] * 3000, global.camY + v[1] * 3000, global.camZ + v[2] * 3000));
...
ray = cm_cast_ray(LEVEL_COLMESH, cm_ray(x, y, z + height, rayx, rayy, rayz));
...
//FIXME test out cm_debug_draw_wireframe
//cm_debug_draw_wireframe(LEVEL_COLMESH);
//FIXME
...
if (bakedColmesh < 0)
{
	...
	cm_vbuff_bake(LEVEL_COLMESH, bakedColmesh, matrix_build_identity(), CM_GROUP_SOLID, 1, 1, c_green);
...
shader_set_uniform_f(shader_get_uniform(sh_cm_debug, "u_color"), 0.4, 0.4, 0.4, 0.4);
cm_vbuff_submit(bakedColmesh);
shader_reset();

//-----------------------------------------------------
//	Draw current region. Bake to vertex buffer if this hasn't been done
var region = cm_get_region(LEVEL_COLMESH, cm_collider_get_aabb(collider));
